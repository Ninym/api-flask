...
def NeteaseDownload(id, ContentType):
    if ContentType == 'attachment':
        ...
        if 'u-errlg u-errlg-404' in stream.text:    # Some VIP songs cannot be downloaded
            print('[NETEASEMUSICDOWNLOAD] Failed while getting the resources and information of a song.'.format(author, song, stream.status_code))
        ...
        if stream.url == "https://music.163.com/404": return {'code': 200, 'link': None, 'name': song, 'author': author, 'msg': 'Cannot fetch download link.'}
...
def FileInfo(id):   # Get the information of the song, including name and author
    ...
    try:
        ...
        if author == '${escape(x.beRepliedUser.nickname)}': author = None
    except:
        author = None
    return song, author
